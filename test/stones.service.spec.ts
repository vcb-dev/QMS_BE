  ...
  describe('importPriceGridRows', () => {
    it('tạo mới đá chưa có trong DB', async () => {
      ...
      const result = await service.importPriceGridRows([
        { stoneType: StoneType.MAIN, name: 'Kim Cương Lab Grown', cut: 'Round', size: '0.7-0.8mm', price: 17082 },
      ]);
      expect(result).toEqual({ imported: 1, updated: 0 });
      expect(prisma.stone.createMany).toHaveBeenCalledWith({
        data: [{ stoneType: StoneType.MAIN, name: 'Kim Cương Lab Grown', cut: 'Round', size: '0.7-0.8mm', price: 17082 }],
      });
    ...
    it('đè giá đá đã có sẵn (trùng stoneType+name+cut+size)', async () => {
      prisma.stone.findMany.mockResolvedValue([
        { id: 'stone-1', stoneType: StoneType.MAIN, name: 'Kim Cương Lab Grown', cut: 'Round', size: '0.7-0.8mm' },
      ]);
      const result = await service.importPriceGridRows([
        { stoneType: StoneType.MAIN, name: 'Kim Cương Lab Grown', cut: 'Round', size: '0.7-0.8mm', price: 20000 },
      ]);
    ...
    it('cùng shape/size trùng nhau ngay trong file — giữ giá dòng sau cùng', async () => {
      ...
      await service.importPriceGridRows([
        { stoneType: StoneType.SIDE, name: 'Kim Cương Lab Grown', cut: 'Round', size: '1.0mm', price: 14001 },
        { stoneType: StoneType.SIDE, name: 'Kim Cương Lab Grown', cut: 'Round', size: '1.0mm', price: 99999 },
      ]);
      expect(prisma.stone.createMany).toHaveBeenCalledWith({
        data: [{ stoneType: StoneType.SIDE, name: 'Kim Cương Lab Grown', cut: 'Round', size: '1.0mm', price: 99999 }],
      });
    });

    it('đá chủ và đá tấm cùng shape/size không tính trùng nhau (stoneType khác)', async () => {
      prisma.stone.findMany.mockResolvedValue([
        { id: 'stone-main', stoneType: StoneType.MAIN, name: 'Kim Cương Lab Grown', cut: 'Round', size: '1.0mm' },
      ]);
      const result = await service.importPriceGridRows([
        { stoneType: StoneType.SIDE, name: 'Kim Cương Lab Grown', cut: 'Round', size: '1.0mm', price: 14001 },
      ]);
  ...
  describe('importPriceGridFromExcel', () => {
    it('parse đúng: Shape→cut, Size→size, Thành tiền→price, tên lấy từ dòng đầu', async () => {
      jest.spyOn(excelService, 'parseExcelFileWithTitleRow').mockReturnValue({
        ...
        rows: [
          { 'Shape': 'Round (Tròn)', 'Size (mm)': '0.7-0.8mm', 'Đơn giá / carat (VND)': 5694000, 'Trọng lượng ước tính (ct)': 0.003, 'Thành tiền (VND)': 17082 },
          { 'Shape': 'Round', 'Size (mm)': '0.9mm', 'Đơn giá / carat (VND)': 5227000, 'Trọng lượng ước tính (ct)': 0.003, 'Thành tiền (VND)': 15681 },
        ],
      });
      prisma.stone.findMany.mockResolvedValue([]);
      ...
      expect(prisma.stone.createMany).toHaveBeenCalledWith({
        data: [
          { stoneType: StoneType.MAIN, name: 'KIM CƯƠNG LAB GROWN', cut: 'Round (Tròn)', size: '0.7-0.8mm', price: 17082 },
          { stoneType: StoneType.MAIN, name: 'KIM CƯƠNG LAB GROWN', cut: 'Round', size: '0.9mm', price: 15681 },
        ],
      });
    });
    ...
    it('thiếu cột Shape/Thành tiền → BadRequestException', async () => {
      ...
      await expect(
        service.importPriceGridFromExcel({ originalname: 'x.xlsx' } as Express.Multer.File, StoneType.MAIN),
    ...
    it('giá không hợp lệ → BadRequestException báo đúng số dòng lỗi', async () => {
      jest.spyOn(excelService, 'parseExcelFileWithTitleRow').mockReturnValue({
        title: 'KIM CƯƠNG LAB GROWN',
        rows: [{ Shape: 'Round', 'Size (mm)': '0.7mm', 'Thành tiền (VND)': 'lỗi' }],
      });
      await expect(
        service.importPriceGridFromExcel({ originalname: 'x.xlsx' } as Express.Multer.File, StoneType.MAIN),
      ).rejects.toThrow(BadRequestException);
