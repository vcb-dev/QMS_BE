...
export class CloudinaryService {
  ...
  async uploadImage(file: Express.Multer.File): Promise<{ url: string; publicId: string }> {
    ...
    if (file.size > maxSizeBytes) {
      throw new BadRequestException('Kích thước tập tin ảnh vượt quá giới hạn cho phép (tối đa 10MB)');
    }
      ...
      const uploadStream = cloudinary.uploader.upload_stream(
        ...
        (error: any, result?: UploadApiResponse) => {
          if (error || !result) {
            ...
            return reject(new BadRequestException(`Không thể tải ảnh lên Cloudinary: ${error?.message || 'Lỗi mạng'}`));
          }
  ...
  async uploadBase64OrUrl(input: string): Promise<string> {
    ...
    if (input.startsWith('data:image')) {
      try {
        ...
        throw new BadRequestException(`Không thể tải ảnh Base64 lên Cloudinary: ${err.message}`);
      }
    }
   ...
   */
  async uploadMultipleImages(files: Express.Multer.File[]): Promise<{ url: string; publicId: string }[]> {
    if (!files || files.length === 0) {
      return [];
    }
    if (files.length > 5) {
      throw new BadRequestException('Chỉ được phép tải lên tối đa 5 ảnh cùng lúc');
    }
