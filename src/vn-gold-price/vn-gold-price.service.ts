...
export class VnGoldPriceService implements OnModuleInit {
  ...
  async onModuleInit() {
    await this.refresh().catch((err) =>
      this.logger.warn(`Không thể tải giá vàng tham khảo lúc khởi động: ${err.message}`),
    );
  }
  ...
  async autoRefresh() {
    try {
      ...
      this.logger.warn(`Tự động refresh giá vàng tham khảo thất bại: ${err.message}`);
    }
  }

  async getLatest(): Promise<{ items: VnGoldPriceItem[]; updatedAt: string; source: string }> {
    // Auto-refresh (Interval) chạy nền 1 ngày/lần — chỗ này chỉ là lưới an toàn
    // nếu vì lý do gì đó dữ liệu vẫn bị cũ hơn dự kiến.
    if (Date.now() - this.lastFetchedAt > APP_CONSTANTS.REFRESH_INTERVAL_MS) {
      try {
        await this.refresh();
      } catch (err: any) {
        this.logger.warn(`Lỗi refresh giá vàng tham khảo, dùng dữ liệu cũ: ${err.message}`);
      }
    }

    return {
      items: this.lastItems,
      updatedAt: this.lastFetchedAt ? new Date(this.lastFetchedAt).toISOString() : new Date().toISOString(),
      source: 'vnappmob.com (nữ trang 99.99%) — chỉ tham khảo, không dùng để tính giá',
    };
  }

  private async ensureToken(): Promise<string> {
    if (this.token && Date.now() - this.tokenFetchedAt < APP_CONSTANTS.TOKEN_TTL_MS) {
      return this.token;
    }
  ...
  private async refresh(): Promise<void> {
    ...
    const data = await this.httpGetJson(APP_CONSTANTS.VNAPPMONEY_GOLD_URL, token);
    ...
    const base24kPerChi = base24kPerLuong / 10; // 1 lượng = 10 chỉ
    ...
    const items: VnGoldPriceItem[] = (config.goldRatios || []).map((r) => {
      const priceVnd = Math.round(base24kPerChi * r.applied);
      nextRaw[r.key] = priceVnd;

      const prev = this.lastRaw[r.key];
      const changeAmount = prev ? priceVnd - prev : null;
      const changePct = prev ? Math.round(((priceVnd - prev) / prev) * 10000) / 100 : null;
