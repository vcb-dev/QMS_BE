...
describe('QuoteQueryService.getDashboardCharts', () => {
...
describe('QuoteQueryService.getStaffPerformance', () => {
  let service: QuoteQueryService;
  let prisma: any;
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        QuoteQueryService,
        { provide: PrismaService, useValue: prisma },
        { provide: QuoteOptionsService, useValue: { batchComputeLivePrices: jest.fn() } },
      ],
    }).compile();
  ...
  it('saleStats gồm cả sale không có đơn nào (total=0)', async () => {
    prisma.user.findMany = jest
      .fn()
      .mockResolvedValueOnce([{ id: 'sale-1', name: 'Sale A' }, { id: 'sale-2', name: 'Sale B' }])
      .mockResolvedValueOnce([]);
    const result = await service.getStaffPerformance();
    expect(result.saleStats).toEqual(
      expect.arrayContaining([
        { id: 'sale-1', name: 'Sale A', total: 3, closed: 2, closeRate: (2 / 3) * 100 },
...
describe('QuoteQueryService.getLibraryProducts', () => {
  let service: QuoteQueryService;
  let prisma: any;
  ...
  beforeEach(async () => {
        ...
        findMany: jest.fn().mockResolvedValue([
          {
            ...
            materials: [{ materialId: 'm1', weightChi: 2, material: { id: 'm1', name: 'Vàng 18K' } }],
            stones: [],
            quoteRequest: {
              id: 'r1',
              code: 'QG-2026-0001',
    ...
    prisma.$queryRaw
      .mockResolvedValueOnce([{ option_id: 'o1', duplicate_count: 1 }]) // trang hiện tại
      .mockResolvedValueOnce([{ count: 1n }]); // tổng
  ...
  it('trả data đã hydrate đủ field + duplicateCount + meta phân trang', async () => {
    const result = await service.getLibraryProducts({ page: 1, limit: 8, sortMode: 'PRICE_DESC' } as any);
    ...
    expect(result.meta).toEqual({ total: 1, page: 1, limit: 8, totalPages: 1 });
  });
});
